...
def fileAppend(fileToTakeFrom, fileToAddTo):
    ...
    while i < addFile_len: 
        # [row, col]
        # If the row and doy and hhmm are equal
        check_add = (df_add[-i,1] == add_last_year) & (df_add[-i,2] == add_last_doy) & (df_add[-i,3] == add_last_hhmm)
        #print(add_var)
    
        
        #print(take_var)
        if check_add:
            compare_add = df_add[-i,1], df_add[-i,2], df_add[-i,3]

            #print((df_take[-i,1]), (df_take[-i,2]), (df_take[-i,3]))
            #print("exists")
