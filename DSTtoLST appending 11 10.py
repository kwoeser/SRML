...
def fileAppend(fileToTakeFrom, fileToAddTo):
    ...
    last_line_year = last_line_take[1]

    last_line_doy = last_line_take[2]

    last_line_hhmm = last_line_take[3]

    df_add = np.array(pd.read_csv(fileAdd, sep=r',|\t',header=None, engine='python'))

    #last_line_add = df_add[-1]
    #FIGURE OUT HOW TO RUN SECTIONS OF CODE ONLY VSC
    
    i = 1
    while i < 100: 
        if (df_add[-i,1] == last_line_year) & (df_add[-i,2] == last_line_doy) & (df_add[-i,3] == last_line_hhmm):
            print((df_add[-i,1]), (df_add[-i,2]), (df_add[-i,3]))
            break

        #print((df_add[-i,1] == last_line_year) & (df_add[-i,2] == last_line_doy) & (df_add[-i,3] == last_line_hhmm))
        i += 1

    

    # last_line_year_add = last_line_add[1]

    # last_line_year_doy = last_line_add[1]
