...
def subroutineadjustagriment(stationname, subtract_rain):
    ...
    pd.DataFrame(df_prn).to_csv(parentfolder + stationname.replace('.dat', '_Agrimet.dat') , header=False, index= False, float_format='%g')
    #np.savetxt(parentfolder + stationname.replace('.dat', '_karma.dat'), df_prn, delimiter=',', fmt='%.18e')
...
# CHANGE NEW FILES TO HAVE _ARGIMET.dat AT THE END (CNO_ARGIEMNT.dat)
