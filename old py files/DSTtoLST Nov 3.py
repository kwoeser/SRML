...
def subroutineadjustagriment(stationname, subtract_rain):
    ...
    if subtract_rain:
        ...
        #np.rain_diff(rain) #prints 0's
        #rain_diff = rain.reshape(len(rain), 0)
        
        rain_diff[0: -1] = np.diff(rain_diff)
        
        print(rain_diff)

        #rain_diff[0:-1] = np.diff()
        #print(rain_diff)
        #rain_diff = np.full((len(rain), 1), diff)
        
        """if rain.size > 0:
            for i in range(len(rain)):
                #rain_diff[i] prints values 
                #rain[i] current pos in file
                if (rain[i] == len(rain) + 1):
                    break
                else:
                    ...
                    #rain = np.diff(rain_diff)

                    #rain[0:-1] = np.diff(rain_diff)
                    rain[0:rain_diff.size] = np.diff(rain_diff)
                    #rain[i] = rain_diff[i + 1] - rain_diff[i]"""
    ...
    if subtract_rain:
        ...
        if rain_diff.size > 0:
            for i in range(len(rain_diff)):
                ...
                #rain_diff = np.append(rain_diff, rain[i + 1] - rain[i])
        else:
            print("size 0")
    ...
    #while 0 < ii:
    while count < ii: #len([0]):#&iteration <100:
        print(ii)
