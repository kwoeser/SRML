...
import srml_makeblankmonth as mbm # Makes the blank month file
...
def subroutineloadsite(sitefilelocation, dat_dt):
    ...
    instrumentelementnumbers, instrumentserialnumbers, instrumentshorthand, instrumentprnmultiplier, instrumentshouldbemultiplier, instrumentuncertainty = mbm.subroutinegetsitefileinfo(sitefile_complete, dat_dt[0])            
    
    instrumentelementnumbersend, instrumentserialnumbersend, instrumentshorthandend, instrumentprnmultiplierend, instrumentshouldbemultiplierend, instrumentuncertaintyend = mbm.subroutinegetsitefileinfo(sitefile_complete, dat_dt[-1])            
    ...
    if np.logical_not(np.array_equal(instrumentshouldbemultiplier.astype(float),instrumentshouldbemultiplierend.astype(float), equal_nan=True)):
        ...
        time.sleep(10)
        sys.exit('line 271')
    #np.logical_not((instrumentserialnumbers == instrumentserialnumbersend).all()):
    #if np.logical_not((instrumentprnmultiplier == instrumentprnmultiplierend).all()):
    #    print('ehere')
    #    np.logical_not((instrumentshorthand == instrumentshorthandend).all()) |\
     #   np.logical_not((instrumentprnmultiplier == instrumentprnmultiplierend).all()) |\
      #  np.logical_not((instrumentshouldbemultiplier == instrumentshouldbemultiplierend).all()) |\
       # np.logical_not((instrumentuncertainty == instrumentuncertaintyend).all()):
...
def subroutinecheckdata(comprehensivefile,compstartrow, dat_dt, dat_data):
    ...
    for idt in range(len(dat_dt)-1):
        ...
        if ((dat_dt[idt+1] - dat_dt[idt]).total_seconds() != timeinterval):
            ...
            print('station: ', station, ' Year//Month: ', comprehensivefile[8,1])
            print(dat_data[idt])
    ...
    print(comprehensivefile[compstartrow:compendrow,2])
...
def subroutineadddatdata(comprehensivefile, compstartrow, compendrow, dat_data, instrumentelementnumbers, instrumentprnmultiplier, instrumentshouldbemultiplier):
    ...
    for icolumnoutput in range(7,comprehensivefile.shape[1],2):
        ...
        #   print(comprehensivefile[8,icolumnoutput])
        # Check to see if the column is measured
        if 'MeasuredColumn' == comprehensivefile[8,icolumnoutput]:
        
    #        print(comprehensivefile[8,icolumnoutput], comprehensivefile[1,icolumnoutput])
    ...
    #                print(instrumentelementnumbers[icolumnsite], outputheader[1,icolumnoutput])
                    #print(instrumentelementnumbers[icolumnsite], instrumentserialnumbers[icolumnsite], instrumentshorthand[icolumnsite], instrumentshouldbemultiplier[icolumnsite], instrumentuncertainty[icolumnsite])                
    #                print(icolumnsite+4)
    #                print(dat_data[:,icolumnsite+4] * instrumentshouldbemultiplier[icolumnsite] / instrumentprnmultiplier[icolumnsite])
    #                outputtemp[:,icolumnoutput] = dat_data[:,icolumnsite+4] * instrumentshouldbemultiplier[icolumnsite] / instrumentprnmultiplier[icolumnsite]
    #                output[startrow]
                    
    #                print(len(comprehensivefile[compstartrow:compendrow]))
     #               print(len(dat_data))
...
comprehensivelocation = 'C:\\Users\\jpeters4\\Dropbox\\updatedformat\\Step4_aftercalibration\\SAO_2022-06_ComprehensiveFormat.csv'

datlocation = 'C:\\Users\\jpeters4\\Dropbox\\updatedformat\\SAO_2022-06.dat'
# use a b c files

sitefilelocation = 'C:\\Users\\jpeters4\\Dropbox\\updatedformat\\SAO_consolodated_sitefile.xlsx'
