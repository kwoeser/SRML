...
def fileAppend(fileToTakeFrom, fileToCompare, fileToAddTo):
    ...
    fileCompare = parentfolder + fileToCompare

    fileAdd = parentfolder + fileToAddTo


    # Load the prn file file to take from
    df_take = np.array(pd.read_csv(fileTake, sep=r',|\t',header=None, engine='python'))
     

    # Finds last line in first file
    take_last_line = df_take[-1]
    #print(take_last_line)

    take_last_year = take_last_line[1]
    take_last_doy = take_last_line[2]
    take_last_hhmm = take_last_line[3]


    """GETTING THE """
    df_compare = np.array(pd.read_csv(fileCompare, sep=r',|\t',header=None, engine='python'))
    
    # Counts the number of lines in df_comapre (ASSUMING THE DATA IS ALL CONNECTED)
    compareFile_len = sum(1 for line in df_compare)

    bottom_rows = np.array([])
    i = 1
    
    while i < compareFile_len:
        # [row, col]
        np.append(bottom_rows, df_compare[-i])
        
        if (df_compare[-i,1] == take_last_year) & (df_compare[-i,2] == take_last_doy) & (df_compare[-i,3] == take_last_hhmm):
            #print((df_compare[-i,1]), (df_compare[-i,2]), (df_compare[-i,3]))
            break

        
        i += 1
    
    # REVERSE THE LIST SINCE IT WAS READ FROM BOTTOM TO TOP
    ...
    addFile_len = sum(1 for line in df_add)
    # # for i in df_add:
    # #     print(i,"\n")
    # print(df_add, "\n")
    # for line in bottom_rows:
    #     np.append(df_add, line)
    # print(df_add)
    # # for i in df_add:
    # #     print(i)
    ...
    print(p)
    while p < addFile_len:
        c = np.insert(df_add, p, bottom_rows, axis = 0)
        p+=1
    print(c, "\n")
    print(df_add)
    ...
    # while i < addFile_len: 
    #     # [row, col]
    #     if (df_add[-i,1] == add_last_year) & (df_add[-i,2] == add_last_doy) & (df_add[-i,3] == add_last_hhmm):
    #         #print((df_add[-i,1]), (df_add[-i,2]), (df_add[-i,3]))
    #         break
    #     i += 1
    ...
    # THEN 
    # Add CHO2 to the end of CHOoutput while not adding any of the things that are already inside of it
    # 
...
fileAppend('CHO1.dat', 'CHO2.dat', 'CHOoutput.dat')
